...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleDrowning,                                  //164 SPELL_AURA_DROWNING
    ...
    &AuraEffect::HandleDetectAmore,                               //170 SPELL_AURA_DETECT_AMORE       various spells that change visual of units for aura target
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //183 SPELL_AURA_MOD_CRIT_CHANCE_ON_TARGETS_ABOVE_HEALTH_PCT implemented in Unit::GetUnitCriticalChance and Unit::GetUnitSpellCriticalChance
    ...
    &AuraEffect::HandleAuraModIncreaseHealthPercent,              //230 SPELL_AURA_MOD_INCREASE_HEALTH_2
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //308 SPELL_AURA_MOD_CRIT_CHANCE_FOR_CASTER implemented in Unit::GetUnitSpellCriticalChance
...
void AuraEffect::HandleDetectAmore(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
        target->SetByteFlag(ACTIVE_PLAYER_FIELD_BYTES2, PLAYER_FIELD_BYTES_2_OFFSET_AURA_VISION, 1 << (GetMiscValue() - 1));
    else
    {
        ...
        target->RemoveByteFlag(ACTIVE_PLAYER_FIELD_BYTES2, PLAYER_FIELD_BYTES_2_OFFSET_AURA_VISION, 1 << (GetMiscValue() - 1));
    }
}
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    SpellPeriodicAuraLogInfo pInfo(this, healInfo.GetHeal(), damage, healInfo.GetHeal() - healInfo.GetEffectiveHeal(), healInfo.GetAbsorb(), 0, 0.0f, crit);
    target->SendPeriodicAuraLog(&pInfo);
...
void AuraEffect::HandleDrowning(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
        target->StartBreathTimer();
    else
        target->StopBreathTimer();
}
